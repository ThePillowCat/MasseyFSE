...
function spider() {
  ...
  if (spiderMoving) {
    ...
    circle(spiderX+5,spiderY-3, 4)
    if(spiderY > crosshairY && spiderY < crosshairY+25 && spiderX+15 > crosshairX && spiderX+15 < crosshairX+25) {
    ...
    if(spiderX > crosshairX && spiderX < crosshairX+20 && spiderY-10 > crosshairY && spiderY-10 < crosshairY+25) {
      generateNewSpiderTimer()
      return
    }
    if(spiderX > crosshairX && spiderX < crosshairX+20 && spiderY+10 > crosshairY && spiderY+10 < crosshairY+25) {
  ...
  for (i = 0; i < boxX1.length; i++) {
    ...
    if(validSquares[i]){
      ...
      for (j=0;j<rectX.length;j++) {
        if (validRects[j]) {
          if (boxX1[i]+28 > rectX[j] && boxX1[i]+28< rectX[j]+rectW2 && boxY1[i]==rectY[j]-12) {
            boxVos[i] = false
          ...
          if (boxX1[i]>rectX[j]&&boxX1[i]<rectX[j]+rectW2&&boxY1[i]==rectY[j]-12) {
            boxVos[i] = true
            //checks if the centipede is moving down or up
            if (boxVosY[i]) {
              boxY1[i]+=28
...
function fire() {
  ...
  if (firing) {
    ...
    for(j=0;j<rectX.length;j++) {
      if (validRects[j]) {
        if (bulletX > rectX[j] && bulletX < rectX[j]+20 && bulletY < rectY[j]+rectH2) {
          firing = false
          validRects[j] = false
        }
      }
    ...
    for (i=0;i<boxX1.length;i++) {
      if (validSquares[i]) {
        if (((bulletX >= boxX1[i] && bulletX <= boxX1[i]+28) || (bulletX+rectW >= boxX1[i] && bulletX+rectW < boxX1[i]+28)) && bulletY >= boxY1[i] && bulletY <= boxY1[i]+28) {
          rectX.push(boxX1[i])
    ...
    if (spiderMoving) {
      if (bulletX>spiderX+2.5 && bulletX<spiderX+30+2.5 && bulletY > spiderY+2.5 && bulletY < spiderY+20+2.5) {
        spiderMoving = false
        firing = false
        generateNewSpiderTimer()
      }
...
function generateNewShipTimer() {
  ...
  setTimeout(()=>{shipMoving = true}, delay2)
}
  
/*
function isPressed(x, y, d) {
  if (dist(x, y, crosshairX, crosshairY) <= d/2 || dist(x, y, crosshairX+20, crosshairY) <= d/2 || dist(x, y, crosshairX, crosshairY+25) <= d/2 || dist(x, y, crosshairX+20, crosshairY+20) <= d/2) {
    fill('#c04adc')
    lives--
    newWave()
  }
